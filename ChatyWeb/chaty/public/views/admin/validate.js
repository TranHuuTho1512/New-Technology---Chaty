...
function ktraSDT() {
    ...
    if (phoneNum.test(sdt)) {
        ...
        document.getElementById("sdtError").innerHTML =
            "Số máy quý khách vừa nhập là số không có thực !!";
        return false;
    }
}
function ktraMK() {
    var Regex = new RegExp(
        "^(?=.{5,})(((?=.*[A-Z])(?=.*[a-z]))|((?=.*[A-Z])(?=.*[0-9]))|((?=.*[a-z])(?=.*[0-9]))).*$",
        "g"
    );
    ...
    if (Regex.test(mk)) {
        ...
        document.getElementById("matKhauError").innerHTML =
            "Vui lòng nhập ít nhất 5 kí tự và kết hợp chữ và số !!";
        return false;
    }
}
...
function ktraSMS() {
    ...
    if (sms === "") {
        document.getElementById("smsError").innerHTML =
            "Vui lòng nhập mã SMS từ tin nhắn điện thoại !!";
        return false;
    } else {
        document.getElementById("smsError").innerHTML = "";
        return true;
